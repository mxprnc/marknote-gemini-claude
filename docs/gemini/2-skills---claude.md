...
**SOP(Standard Operating Procedure, 표준 작업 절차)**는 특정 업무를 수행할 때 누가 하더라도 동일한 높은 품질의 결과를 얻을 수 있도록 정리해둔 **단계별 지침서**입니다.

#### 1. 주요 개념
- **일관성**: 작업자(또는 AI)가 바뀌어도 동일한 프로세스를 따르므로 결과가 일정합니다.
- **효율성**: 매번 "어떻게 할까?" 고민하지 않고 정해진 절차에 집중할 수 있습니다.
- **품위 유지**: 최소한의 품질 하한선을 보장하며 실수를 방지합니다.

#### 2. AI Skill에서의 의미
- **매뉴얼(SKILL.md)**: AI에게 "이 지식(Reference)을 참고해서, 이런 순서(SOP)와 도구(Scripts)로 행동해"라고 알려주는 핵심 로직입니다.
- **프로세스 중심**: 지식을 나열하는 것이 아니라, **'어떤 순서로 일을 처리할 것인가'**를 정의하는 것이 SOP의 핵심입니다.

<br/>

### skill 사용시 주의사항
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-1.png)
- '절차' 와 '지식'을 분리
- '적게' 만들되 '제대로'
- '리팩토링' : 다운로드 받은 경우 내 비즈니스에 맞도록 리팩토링

<br/>

많은 종류의 skill 보다 꼭 필요한 skill 20 \~ 30 개가 더 낫다.
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-2.png)

<br/>

### 권장되는 skill 디렉터리 구조 
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-3.png)
<br/>

- SKILL.md : `SOP` (Standard Operating Procedure)
  - 클로드에게 알려주는 업무 매뉴얼
- references/ : 지식
- scripts/ : 도구
...
### skill 작성시 3가지 핵심요소
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-4.png)
- '프로세스 먼저' : skill.md 에 순서를 작성
- '트리거' 작성 : front matter 에 사용조건 정의 (언제 어디서 어떤 조건에 발동될지 명확하게 정의)
- '전문성' 분리 : 레퍼런스 또는 스크립트로 분리

<br/>


### SKILL.md 파일의 권장 구조
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-5.png)

<br/>

### 레퍼런스
레퍼런스 추가 시 주의점 : "쏟아붓지 말고 가리켜라."
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-6.png)

- 다 때려넣고 길게 설명을 작성하지 말고 필요한 것들을 '가리켜야'한다.

<br/>

나쁜예
![alt text](./img/2-skills---claude-bast-practice/summary-claude-skill-7.png)
